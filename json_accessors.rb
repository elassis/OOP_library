    ...
    if File.exists?("#{@type}.json")
      ...
      when 'people'
        array_object = array_hashes.map { 
          |people| (people['class'] == 'Student') ? Student.new(age: people['age'], name: people['name'], parent_permission: people['permission'], id: people['id']) :
          Teacher.new(age: people['age'], name: people['name'], specialization: people['spec'],parent_permission: people['permission'], id: people['id']) 
        }
      when 'rental'
        array_object = array_hashes.map{|rental| Rental.new(rental['date'], list_book.detect {|b| b.id == rental['book']},list_people.detect {|p| p.id == rental['person']}
        )}
      end
      return array_object
    end
